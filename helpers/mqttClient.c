                                 ...
                                 int persistence_type, void *persistence_context, MQTTClient_createOptions *options)
{
    ...
    {
        if (strncmp(URI_TCP, serverURI, strlen(URI_TCP)) != 0 && strncmp(URI_WS, serverURI, strlen(URI_WS)) != 0
#if defined(OPENSSL)
            && strncmp(URI_SSL, serverURI, strlen(URI_SSL)) != 0 && strncmp(URI_WSS, serverURI, strlen(URI_WSS)) != 0
#endif
        )
        {
...
static thread_return_type WINAPI MQTTClient_run(void *n)
{
    ...
    while (!tostop)
    {
        ...
        else
        {
            ...
            {
                rc = m->c->sslopts->struct_version >= 3 ? SSLSocket_connect(m->c->net.ssl, m->c->net.socket, m->serverURI,
    ...
    if (m->c->connect_state == TCP_IN_PROGRESS) /* TCP connect started - wait for completion */
    {
        ...
        if (m->ssl)
        {
            ...
            if (setSocketForSSLrc != MQTTCLIENT_SUCCESS)
            {
                ...
                rc = m->c->sslopts->struct_version >= 3 ? SSLSocket_connect(m->c->net.ssl, m->c->net.socket, serverURI,
                                                                            m->c->sslopts->verify, m->c->sslopts->ssl_error_cb, m->c->sslopts->ssl_error_context)
    ...
    if (*sock > 0 && rc1 == 0)
    {
        ...
        if (pack)
        {
            ...
            else if (pack->header.bits.type == PUBACK || pack->header.bits.type == PUBCOMP)
            {
                ...
                *rc = (pack->header.bits.type == PUBCOMP) ? MQTTProtocol_handlePubcomps(pack, *sock) : MQTTProtocol_handlePubacks(pack, *sock);
                if (m && m->dc)
                {
